...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    l = len(IDandRatingsTuple[1])
    s = sum(IDandRatingsTuple[1])
    return (IDandRatingsTuple[0],(l, float(s)/l))
...
# TODO: Replace <FILL IN> with appropriate code
l = [i for (x,y,i) in myRatedMovies]
...
myUnratedMoviesRDD = (moviesRDD.filter(lambda x: x[0] not in l)
                      .map(lambda x: (0,x[0])))
