...
parsedSamplePoints = [parsePoint(p) for p in samplePoints]
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return sqrt(labelsAndPreds.map(lambda (x,y): squaredError(x,y)).mean())
...
def twoWayInteractions(lp):
    ...
    """
    mergedlist = [x for x in lp.features] + [x1*x2 for x1 in lp.features for x2 in lp.features]
    return LabeledPoint(lp.label, mergedlist)
