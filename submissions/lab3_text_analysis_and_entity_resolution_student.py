...
def findBiggestRecord(vendorRDD):
    ...
    """
    return [vendorRDD.reduce(lambda x,y: x if len(x[1]) > len(y[1]) else y)]
...
def tf(tokens):
    ...
    """
    tokensLen = float(len(tokens))
    return {token:sum([token==t for t in tokens])/tokensLen for token in tokens}
...
def idfs(corpus):
    ...
    uniqueTokens = corpus.flatMap(lambda x: [(a,1) for a in set(x[1])])
    tokenCountPairTuple = uniqueTokens.reduceByKey(lambda x,y: x+y)
    tokenSumPairTuple = tokenCountPairTuple.map(lambda x: (x[0],N/x[1]))
    return (tokenSumPairTuple.cache())
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {k:tfs[k]*idfs[k] for k in tfs.keys()}
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    return sum([a[k]*b[k] for k in a.keys() if k in a and k in b])
...
avgSimNon = nonDupsRDD.map(lambda x: x[1]).reduce(lambda x,y: x+y) / float(nonDupsRDD.count())
...
idfsFullWeights = {k:v for (k,v) in idfsFull.collect()}
idfsFullBroadcast = sc.broadcast(idfsFullWeights)

#print idfsFullWeights["paperless"]
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = {k:norm(v) for (k,v) in amazonWeightsRDD.collect()}
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = {k:norm(v) for (k,v) in googleWeightsRDD.collect()}
...
def invert(record):
    ...
    """
    pairs = [(k,record[0]) for k in record[1].keys()]
    return (pairs)
...
amazonWeightsDict = {k:v for (k,v) in amazonWeightsRDD.collect()}
googleWeightsDict = {k:v for (k,v) in googleWeightsRDD.collect()}
#print amazonWeightsDict['b000jz4hqo']
#print amazonNormsBroadcast.value['b000jz4hqo']
amazonWeightsBroadcast = sc.broadcast(amazonWeightsDict)
googleWeightsBroadcast = sc.broadcast(googleWeightsDict)
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum([amazonWeightsBroadcast.value[amazonRec][x]*googleWeightsBroadcast.value[googleRec][x] for x in tokens])
